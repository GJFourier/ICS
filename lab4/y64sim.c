...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    bool_t ovf = FALSE;
	if(argB > 0 && argA > 0 && op == A_ADD && val < 0)
		ovf = TRUE;
	else if(argB > 0 && argA < 0 && op == A_SUB && val < 0)
		ovf = TRUE;
	else if(argB < 0 && argA < 0 && op == A_ADD && val > 0)
		ovf = TRUE;
	else if(argB < 0 && argA > 0 && op == A_SUB && val > 0)
		ovf = TRUE;
...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch (icode) {
      ...
      case I_RRMOVQ:  /* 2:x regA:regB */
		...
		reg_value = get_reg_val(sim->r, regA);
		if(cond_doit(sim->cc, ifun))
			set_reg_val(sim->r, regB, reg_value);
		sim->pc = next_pc;
		break;
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
		...
		regA = GET_REGA(registers);
		regB = GET_REGB(registers);
		reg_value = get_reg_val(sim->r, regB);
		address = reg_value + immediate;
		reg_value = get_reg_val(sim->r, regA);
		set_long_val(sim->m, address, reg_value);
		sim->pc = next_pc;
		break;
      case I_MRMOVQ: /* 5:0 regB:regA imm */
		...
		reg_value = get_reg_val(sim->r, regB);
		address = reg_value + immediate;
		if (!get_long_val(sim->m, address, &immediate)) {
        	err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, address);
        	return STAT_ADR;
        }
		set_reg_val(sim->r, regA, immediate);
		sim->pc = next_pc;
		break;
      case I_ALU: /* 6:x regA:regB */
		...
		regA = GET_REGA(registers);
		regB = GET_REGB(registers);
		reg_value = compute_alu(ifun, get_reg_val(sim->r, regA), get_reg_val(sim->r, regB));
		set_reg_val(sim->r, regB, reg_value);
		sim->cc = compute_cc(ifun, get_reg_val(sim->r, regA), get_reg_val(sim->r, regB), reg_value);
		sim->pc = next_pc;
		break;		
      case I_JMP: /* 7:x imm */
		if (!get_long_val(sim->m, next_pc, &address)) {
        	err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
        	return STAT_ADR;
        }
		next_pc += 8;
      ...
      case I_CALL: /* 8:x imm */
		...
		next_pc += 8;
		reg_value = get_reg_val(sim->r, REG_RSP);
		reg_value -= 8;
		set_reg_val(sim->r, REG_RSP, reg_value);
		if (!set_long_val(sim->m, reg_value, next_pc)) {
        	err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, reg_value);
        	return STAT_ADR;
        }
		sim->pc = address;
		break;
      case I_RET: /* 9:0 */
		reg_value = get_reg_val(sim->r, REG_RSP);
		if (!get_long_val(sim->m, reg_value, &address)) {
        	err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
        	return STAT_ADR;
        }
		reg_value += 8;
      ...
      case I_PUSHQ: /* A:0 regA:F */
		...
		next_pc++;
		regA = GET_REGA(registers);
		immediate = get_reg_val(sim->r, regA);
		reg_value -= 8;
		set_reg_val(sim->r, REG_RSP, reg_value);
		if (!set_long_val(sim->m, reg_value, immediate)) {
        	err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, reg_value);
        	return STAT_ADR;
        }
		sim->pc = next_pc;
      ...
      case I_POPQ: /* B:0 regA:F */
		reg_value = get_reg_val(sim->r, REG_RSP);
		if (!get_long_val(sim->m, reg_value, &immediate)) {
        	err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
        	return STAT_ADR;
        }
		reg_value += 8;
